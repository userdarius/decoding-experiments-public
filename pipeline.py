...
def score_pipeline(base_gen_model, entailment_model, dataset,
                   num_fewshot=200, num_generations=10, brief="", brief_always=True, return_indices=True,
                   prompt="", save_path="result.pkl", p_true_few_shot_prompt="", use_context=False, cot=False):
    ...
    for index in tqdm(indices):
        ...
        print("-------------------------")
        if cot:
            local_prompt = make_prompt(
                context, question, None, brief, True, cot=True)
        else:
        ...
        p_true = calculate_p_true(
            base_gen_model, question, most_likely_answer_dict['response'],
        ...
        log_str = f'semantic_ids: {semantic_ids}, avg_token_log_likelihoods: {log_liks_agg}'
        print(log_str)
        print("Entropies:")
